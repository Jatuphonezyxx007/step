    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-20">
        <div className="flex items-center gap-2">
          ...
          <Dropdown>
            <DropdownTrigger>
              <Button variant="bordered" className="bg-gray-100 hover:bg-gray-200 px-3 py-2 rounded-lg flex items-center gap-2 transition-all">
                หมวดหมู่สินค้า
              </Button>
            </DropdownTrigger>
            <DropdownMenu aria-label="หมวดหมู่สินค้า">
              {categories.map((category, index) => (
                <DropdownItem key={index} className="hover:text-blue-600 cursor-pointer">
                  {category}
                </DropdownItem>
              ))}
              <DropdownItem key="profile" className="border-t pt-2 flex justify-center">
                <motion.div
