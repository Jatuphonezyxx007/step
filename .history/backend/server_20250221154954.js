...
//   } catch (error) {
//     console.error('❌ Database Connection Failed:', error);
//     // 🔥 ส่งข้อผิดพลาดกลับไปยังฝั่ง Client
...
// ✅ Endpoint เพื่อตรวจสอบสถานะการเชื่อมต่อฐานข้อมูล
app.get('/api/status', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT 1 AS status_check');
    if (rows) {
      res.status(200).json({ success: true, message: '✅ เชื่อมต่อกับฐานข้อมูลเรียบร้อยแล้ว' });
    }
  } catch (error) {
