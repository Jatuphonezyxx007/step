...
const storageProducts = multer.diskStorage({
  destination: path.join(__dirname, "../admin/public/products"), // เก็บรูปภาพในโฟลเดอร์นี้
  filename: (req, file, cb) => {
    const { product_id, index } = req.params; // รับค่า product_id และ index ของรูป
    const ext = path.extname(file.originalname); // นามสกุลไฟล์
    cb(null, `${product_id}_${index}${ext}`); // ตั้งชื่อไฟล์เป็น "product_id_index.ext"
  },
});
...
app.put('/api/products/:product_id', async (req, res) => {
  ...
  try {
    ...
    // อัปเดตข้อมูลสินค้า (ชื่อสินค้า & หมวดหมู่)
...
app.put("/api/upload/image/:product_id/:index", uploadProductImages.single("image"), (req, res) => {
  if (!req.file) {
