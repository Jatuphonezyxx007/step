...
app.put('/api/products/:product_id', async (req, res) => {
  try {
    const { product_id } = req.params;
    const { product_name, category_id, series_id, images_main, detail } = req.body;
    const query = `
      UPDATE products
      SET product_name =?, category_id =?, series_id =?, images_main =?, detail =?
      WHERE product_id =?
    `;
    const values = [product_name, category_id, series_id, images_main, detail, product_id];
