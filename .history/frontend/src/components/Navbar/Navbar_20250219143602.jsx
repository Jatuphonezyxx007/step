// import React from 'react';
// import { Link } from 'react-router-dom';
// import './Navbar.css';
// import logo from '../../assets/logo/step-solutions-logo.png';

// export default function Navbar() {
//   return (
//     <nav className="navbar">
//       <div className="navbar-container">
//         <div className="navbar-logo">
//           <Link to="/">
//             <img src={logo} alt="Step Solutions Logo" />
//           </Link>
//         </div>

//         <div className="category-button">
//           <svg
//             xmlns="http://www.w3.org/2000/svg"
//             className="h-6 w-6"
//             fill="none"
//             viewBox="0 0 24 24"
//             stroke="currentColor"
//           >
//             <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" />
//           </svg>
//           <span>หมวดหมู่สินค้า</span>
//         </div>

//         <div className="search-bar">
//           <input type="text" placeholder="ค้นหาสินค้า" />
//         </div>

//         <div className="navbar-links">
//           <Link to="/" className="navbar-link active">
//             หน้าแรก
//           </Link>
//           <Link to="/computers" className="navbar-link">
//             คอมพิวเตอร์เซ็ต
//           </Link>
//           <Link to="/accessories" className="navbar-link">
//             อุปกรณ์คอม
//           </Link>
//           <Link to="/blog" className="navbar-link">
//             บทความ
//           </Link>
//           <Link to="/contact" className="navbar-link">
//             ติดต่อเรา
//           </Link>
//         </div>

//         <div className="profile-icon">
//           <svg
//             xmlns="http://www.w3.org/2000/svg"
//             className="h-6 w-6"
//             fill="none"
//             viewBox="0 0 24 24"
//             stroke="currentColor"
//           >
//             <path
//               strokeLinecap="round"
//               strokeLinejoin="round"
//               strokeWidth="2"
//               d="M5.121 17.804A9 9 0 0112 15a9 9 0 016.879 2.804M15 11a3 3 0 11-6 0 3 3 0 016 0z"
//             />
//           </svg>
//         </div>
//       </div>
//     </nav>
//   );
// }


import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import { Avatar } from '@heroui/react';
import logo from '../../assets/logo/step-solutions-logo.png';
import './Navbar.css';

export default function Navbar() {
  const [bgColor, setBgColor] = useState('white');

  const handleBgChange = (color) => {
    setBgColor(color);
  };

  return (
    <>
      <motion.nav
        className="navbar"
        style={{ backgroundColor: bgColor }}
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        transition={{ duration: 0.5, ease: 'easeOut' }}
      >
        <div className="navbar-container">
          <div className="navbar-left">
            <Link to="/" className="navbar-logo">
              <motion.img
                src={logo}
                alt="Step Solutions Logo"
                className="logo-image"
                whileHover={{ scale: 1.1 }}
                transition={{ type: 'spring', stiffness: 300 }}
              />
            </Link>
          </div>

          <div className="search-bar">
            <span className="material-symbols-outlined search-icon">search</span>
            <input
              type="text"
              placeholder="ค้นหาสินค้า"
              className="search-input"
            />
          </div>

          <div className="profile-icon hidden-mobile">
            <Avatar
              showFallback
              src="https://images.unsplash.com/broken"
              className="w-10 h-10 rounded-full cursor-pointer"
            />
          </div>
        </div>

        <div className="bg-color-selector">
          <button onClick={() => handleBgChange('white')}>ขาว</button>
          <button onClick={() => handleBgChange('#f3f4f6')}>เทาอ่อน</button>
          <button onClick={() => handleBgChange('#1d4ed8')}>น้ำเงิน</button>
          <button onClick={() => handleBgChange('#ef4444')}>แดง</button>
        </div>
      </motion.nav>

      {/* Bottom Navbar for Mobile Only */}
      <div className="bottom-navbar">
        <Link to="/" className="bottom-navbar-item active">
          <span className="material-symbols-outlined">home</span>
          <span>หน้าแรก</span>
        </Link>
        <Link to="/categories" className="bottom-navbar-item">
          <span className="material-symbols-outlined">grid_view</span>
          <span>หมวดหมู่</span>
        </Link>
        <Link to="/customize" className="bottom-navbar-item">
          <span className="material-symbols-outlined">build</span>
          <span>จัดสเปกคอม</span>
        </Link>
        <Link to="/cart" className="bottom-navbar-item">
          <span className="material-symbols-outlined">shopping_bag</span>
          <span>ตะกร้า</span>
        </Link>
        <Link to="/account" className="bottom-navbar-item">
          <Avatar
            showFallback
            src="https://images.unsplash.com/broken"
            className="w-8 h-8 rounded-full cursor-pointer"
          />
          <span>บัญชี</span>
        </Link>
      </div>
    </>
  );
}
